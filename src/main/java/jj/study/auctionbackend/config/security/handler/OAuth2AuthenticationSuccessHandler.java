...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    protected String determineTargetUrl(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        try {
            ...
            AuthToken accessToken = authTokenProvider.createAuthToken(userInfo.getId(), roleType.getCode(),
                    DateFormat.getDateInstance().parse(String.valueOf(accessTokenExpirationTime))
            );
            AuthToken refreshToken = authTokenProvider.createAuthToken(customJwtProperties.getSecret(),
                    DateFormat.getDateInstance().parse(String.valueOf(refreshTokenExpirationTime))
            );
            // (12)
            ...
            CustomCookieUtil.addCookie(response, customJwtProperties.getRefreshToken(),
                    refreshToken.getToken(), Math.toIntExact(refreshTokenExpirationTime / 60)
            );
            // (14)
            return UriComponentsBuilder.fromUriString(targetUrl).queryParam("token", accessToken.getToken())
                    .build().toUriString();
        }
        // (10-catch)
        catch (ParseException e) {
            log.info("An error occurred. Message := {}", e.getLocalizedMessage());
            throw new CustomTokenException("An error occurred while parsing the expiration date");
        }
    }
